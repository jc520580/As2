...
public class SimulationPanel extends JPanel {
    ...
    private ArrayList<Verhricle> vehicles = new ArrayList<>();
    ...
    private void createVehicle() {
        ...
        switch (randomVehicle) {
            ...
            case 1:
                ...
                break;
            case 2:
                vehicles.add(new Motor(Integer.toString(cycle), roads.get(0)));
                break;
        }
    }
    ...
    public void simulate() {
        ...
        timer = new Timer(updateRate / 60, e -> {
            ...
            for (Iterator<Verhricle> iterator = vehicles.iterator(); iterator.hasNext(); ) {
                Verhricle vehicle = iterator.next();
//                vehicle.setLane(Model.Vehicle.Lane.LEFT);
                vehicle.Move();
                vehicle.printStatus();
                if (vehicle.getPosition() + vehicle.getLength() + vehicle.getSpeed() >= vehicle.getCurrentRoad().getLength() && vehicle.getCurrentRoad().getConnectedRoad().isEmpty() && (vehicle.getSpeed() == 0)) {
                    vehicle.getCurrentRoad().getVerhriclesOnRoad().remove(vehicle);
                    iterator.remove();
                    vehiclesRemoved++;
                }
            }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (!vehicles.isEmpty()) {
            for (Verhricle vehicle : vehicles
            ) {
